...
public class Main {
	...
	public static HeroFactory heroFactory = HeroFactory.getInstance();
		...
		while(true) {
			...
			case "add":
				...
				try {
					...
					Hero newHero = heroFactory.createHero(heroType, cmdTokens[2]);
					...
					int posY = 0;
					do {
						posX = generator.nextInt(world.MAP_SIZE);
						posY = generator.nextInt(world.MAP_SIZE);
					} while (map[posY][posX] != 0);

					newHero.setPosx(posX);
					newHero.setPosy(posY);

					world.addHeroToMap(posX,posY);
					world.getParty().add(newHero);
